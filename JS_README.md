...
Anime.js and GSAP adapters preserve step easing for ordinary tweens and keyframes. Timeline replacement that cuts through a step segment is rejected with `unsupported_overlap_easing` until the compiler can expand every discontinuity with the correct endpoint rule. Compilable WebCompat `@keyframes` rules use the same MovieScene timing IR, including `steps()` and arbitrary `cubic-bezier()`.

Native CSS activation is observed below `URmlUiJSContext`. Host ABI 7 emits synchronous View-level notifications from every public Bridge tree mutation entry point that can affect the current class/id selectors or replace a subtree. The callback only records a deduplicated dirty batch and requests a wake; selector matching, invalid-handle cleanup, binding creation, and cancellation run after JS advance. This means typed Vue calls and direct native `RmlUE_*` or `URmlUiWidget` mutations share one activation path without duplicate JSContext notifications.
