...
The opt-in Slate command renderer avoids the full-view render target and replays strict 2D affine transforms and the final intersection of nested rectangular scissor regions. ABI v4 publishes geometry and texture create/destroy deltas keyed by stable IDs, so unchanged frames reuse UE-side caches instead of copying complete resources again. Ordinary color and generated-texture draws use `ICustomSlateElement` RDG passes backed by persistent static RHI vertex/index buffers; creation and deferred destruction run through Unreal's render-resource lifecycle. UE UI material draws deliberately remain on Slate `MakeCustomVerts`, preserving the normal `MD_UI` material pipeline and their ordering with RHI draws. Perspective/3D transforms, rounded or non-rectangular clip masks, layers, filters and RmlUi shaders remain unsupported on that path and are reported through the frame feature mask.
