...
Tests are registered as `RmlUiUnreal.*`. `RmlUiUnreal.Slate.RhiDirectRendering` verifies actual RHI output, CSS color space, premultiplied alpha, nested rounded clip-mask pixels, cache readiness, and recursive teardown on both DX11 and DX12 hosts. The sample project's root README contains launch, packaging and test commands. Upstream licenses are retained in vendored source and `Content/RmlUi`.
