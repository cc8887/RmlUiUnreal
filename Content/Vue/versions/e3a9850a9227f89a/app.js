...
async function save() {
  ...
  state.status = "Saving";
  try {
    const response = await callHost("saveSession", { name: state.name, projects: state.projects.length });
    state.saves++;
...
var __component2 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props, { expose: __expose }) {
    ...
    onMounted(() => {
      ...
      void callHost("getHostInfo", {}).then((host) => {
        hostName.value = host.name;
      }).catch(() => {
      });
    });
    onUnmounted(() => clearInterval(timer));
    const __returned__ = { total, hostName, get timer() {
      return timer;
    }, set timer(v) {
      timer = v;
    }, get state() {
