...
var __component = /* @__PURE__ */ defineComponent({
  ...
  setup(__props, { expose: __expose }) {
    ...
    const lastUpdated = ref("Waiting for world");
    const error = ref("");
    let timer = 0;
    const types = computed2(() => ["All", ...Array.from(new Set(snapshot.value.actors.map((actor) => actor.type))).sort()]);
    const visibleActors = computed2(() => {
      const needle = query.value.trim().toLowerCase();
      return snapshot.value.actors.filter((actor) => (typeFilter.value === "All" || actor.type === typeFilter.value) && (!needle || actor.name.toLowerCase().includes(needle) || actor.type.toLowerCase().includes(needle)));
    });
    ...
    function refresh() {
      try {
        ...
        lastUpdated.value = `Live / ${next.sequence}`;
        error.value = "";
      } catch (caught) {
    ...
    const __returned__ = { observer, snapshot, selectedPath, showDetails, details, query, typeFilter, lastUpdated, error, get timer() {
      return timer;
    }, set timer(v) {
      timer = v;
    }, types, visibleActors, selected, tickingCount, parse, refresh, select, toggleDetails };
    Object.defineProperty(__returned__, "__isScriptSetup", { enumerable: false, value: true });
    return __returned__;
  }
});
function render(_ctx, _cache, $props, $setup, $data, $options) {
  return openBlock(), createElementBlock("div", { class: "flex h-full w-full flex-col bg-slate-100 text-ink" }, [
    createBaseVNode("div", { class: "flex items-center justify-between border-b border-slate-300 bg-white px-6 py-4" }, [
      ...
      createBaseVNode("div", { class: "flex items-center gap-3" }, [
        createBaseVNode(
          "span",
          { class: "text-xs text-signal" },
          toDisplayString($setup.lastUpdated),
          1
          /* TEXT */
        ),
        createBaseVNode("button", {
          ...
          createBaseVNode("img", { src: "icons/refresh-cw.png" })
        ])
      ])
    ]),
    createBaseVNode("div", { class: "grid grid-cols-3 gap-4 border-b border-slate-300 bg-slate-50 px-6 py-4" }, [
      ...
      createBaseVNode("div", null, [
        ...
        createBaseVNode(
          "p",
          { class: "metric-value" },
          toDisplayString($setup.types.length - 1),
          1
          /* TEXT */
        )
      ]),
    ...
    createBaseVNode("div", { class: "grid grid-cols-2 gap-3 border-b border-slate-300 bg-white px-6 py-3" }, [
      ...
      withDirectives(createBaseVNode(
        ...
        {
          id: "actor-type",
          "onUpdate:modelValue": _cache[1] || (_cache[1] = ($event) => $setup.typeFilter = $event),
          class: "control"
        },
        [
