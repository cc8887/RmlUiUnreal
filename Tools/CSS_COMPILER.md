...
The repository-wide no-silent-failure rule is defined in [the plugin constraints](../AGENTS.md). A successful conversion must preserve the requested semantic or report the exact fallback route; an unrepresentable declaration must fail before publishing output. This also applies to conversions that compile successfully but cannot bind or execute at runtime.

Native CSS animation manifests retain `animation-play-state` rules from every inline `<style>` block and local linked stylesheet. A strict inline `style` attribute with a native animation control is rejected (`unsupported-inline-motion-manifest`) because it has no stable selector for the manifest. Native animation rules using pseudo states or sibling combinators are rejected (`unsupported-native-animation-selector`): the activation observer cannot invalidate them on input-state or sibling changes. Keyframes and animation declarations must share a CSS compilation unit for native routing; otherwise `native-keyframes-not-in-source` reports an RmlUi RCSS fallback, which is not a MovieScene binding.
