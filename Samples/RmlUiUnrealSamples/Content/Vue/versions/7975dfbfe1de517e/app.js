...
var RmlNode = class {
  ...
  get isConnected() {
    return native.IsNodeValid(this.handle) && native.ContainsNode(native.RootNode(), this.handle);
  }
  get parentElement() {
    return wrap(native.ParentNode(this.handle));
  }
  get childNodes() {
    return JSON.parse(native.ChildNodes(this.handle)).map((handle) => wrap(handle));
  }
  getAttribute(name) {
    return native.GetAttribute(this.handle, name);
  }
  setAttribute(name, value) {
    check(native.SetAttribute(this.handle, name, value, false));
  }
  removeAttribute(name) {
    check(native.SetAttribute(this.handle, name, "", true));
  }
  get classList() {
    const contains = (name) => this.getAttribute("class").split(/\s+/).includes(name);
    return {
      contains,
      add: (...names) => {
        for (const name of names) check(native.SetNodeClass(this.handle, name, true));
      },
      remove: (...names) => {
        for (const name of names) check(native.SetNodeClass(this.handle, name, false));
      },
      toggle: (name, force) => {
        const enabled = force ?? !contains(name);
        check(native.SetNodeClass(this.handle, name, enabled));
        return enabled;
      }
    };
  }
  querySelector(selector) {
    return wrap(native.QueryNode(this.handle, selector));
  }
  querySelectorAll(selector) {
    return JSON.parse(native.QueryNodes(this.handle, selector)).map((handle) => wrap(handle));
  }
  contains(node) {
    return native.ContainsNode(this.handle, node.handle);
  }
  focus() {
    check(native.FocusNode(this.handle));
  }
  blur() {
    check(native.BlurNode(this.handle));
  }
  capturePointer(pointerId = 0) {
    check(native.CaptureNode(this.handle, pointerId));
  }
  releasePointer(pointerId = 0) {
    check(native.ReleaseCaptureNode(this.handle, pointerId));
  }
};
...
function listen(node, key, type, callback, capture = false, once = false) {
  ...
  listeners.set(id, { node, callback, once, key, type });
}
...
function dispatchCssAnimationBatch(json) {
  try {
    const batch2 = JSON.parse(json);
    if (batch2?.type !== "css-animation-events" || !Array.isArray(batch2.events)) return;
    for (const data of batch2.events) {
      ...
      while (current) {
        for (const listener of [...listeners.values()]) {
          ...
          const event = {
            ...
            target: wrap(Number(data.target)),
            ...
            keyName: "",
            button: 0,
            phase: current.handle === Number(data.target) ? 2 : 3,
            x: 0,
            y: 0,
            modifiers: 0,
            code: "",
...
function disposeRenderer() {
  ...
  native.OnHostEvent?.Remove(dispatchCssAnimationBatch);
}
function kebab(name) {
...
function equalValue(a, b) {
  return Object.is(a, b) || a != null && b != null && typeof a !== "object" && typeof b !== "object" && String(a) === String(b);
}
function controlKind(node) {
  return node.tag === "select" ? "select" : node.props.modelVNode?.props?.type || node.props.type || "text";
}
function checkboxValue(node, checked) {
...
function commitModel(node, value, checked) {
  ...
  if (kind === "checkbox") {
    if (Array.isArray(binding.value)) {
      const next = binding.value.filter((item) => !equalValue(item, option));
      if (checked) next.push(option);
      assign(vnode, next);
    } else if (binding.value instanceof Set) {
      const next = new Set(binding.value);
      if (checked) next.add(option);
      else for (const item of next) if (equalValue(item, option)) next.delete(item);
      assign(vnode, next);
    } else assign(vnode, checkboxValue(node, checked));
  } else if (kind === "radio") {
    if (checked) assign(vnode, option);
  } else {
    let result = binding.modifiers.trim ? value.trim() : value;
    if (binding.modifiers.number || kind === "number") {
...
function syncModel(node) {
  ...
  if (kind === "radio" || kind === "checkbox") {
    ...
    const checked = kind === "radio" ? equalValue(value, option) : Array.isArray(value) || value instanceof Set ? [...value].some((item) => equalValue(item, option)) : equalValue(value, checkboxValue(node, true));
...
var modelDirective = {
  created(node, binding, vnode) {
    ...
    node.props.modelBinding = binding;
    ...
    listen(node, "$model-composition-start", "compositionstart", () => {
      node.props.composing = true;
    });
    ...
    listen(node, "$model-blur", "blur", () => {
      if (!node.props.composing && node.props.modelBinding.modifiers.lazy) commit();
    });
  ...
  updated: syncModel,
  beforeUnmount(node) {
    for (const key of [...node.events.keys()]) if (key.startsWith("$model")) unlisten(node, key);
  }
};
var vModelText = modelDirective;
var vModelCheckbox = modelDirective;
var vModelSelect = modelDirective;
function withModifiers(fn, modifiers) {
  return (event) => {
    for (const modifier of modifiers) {
      ...
      else if (modifier === "exact" && [["ctrl", 2], ["shift", 1], ["alt", 4], ["meta", 32]].some(([name, bit]) => !modifiers.includes(name) && event.modifiers & bit)) return;
...
// src/vue/store.ts
var host = findService("host");
var initial = {
  name: "Northstar session",
  enabled: true,
  accent: "teal",
  intensity: 72,
  projects: [{ id: 1, name: "Interface assembly", progress: 82 }, { id: 2, name: "Style validation", progress: 64 }, { id: 3, name: "Asset synchronization", progress: 46 }],
  nextId: 4,
  saves: 0
};
var restored = {};
try {
  const value = JSON.parse(native.StateJson);
  if (value.schema === 1) restored = value.data;
} catch {
}
var state = reactive({ ...initial, ...restored, busy: false, status: "Ready", events: 0, ticks: 0 });
var version2 = native.Version;
function addProject() {
  const id = state.nextId++;
  state.projects.push({ id, name: `Project ${id}`, progress: 20 });
  state.events++;
}
...
function serialize() {
  return JSON.stringify({ schema: 1, data: {
    name: state.name,
...
watchEffect(() => native.ReportDebugState(JSON.stringify({ ...state, version: version2 })));

// src/vue/ProjectCard.vue
var __component = /* @__PURE__ */ defineComponent({
  ...
  setup(__props, { expose: __expose, emit: __emit }) {
    __expose();
    const emit2 = __emit;
    const __returned__ = { emit: emit2 };
    Object.defineProperty(__returned__, "__isScriptSetup", { enumerable: false, value: true });
    return __returned__;
  }
});
function render(_ctx, _cache, $props, $setup, $data, $options) {
  return openBlock(), createElementBlock("div", { class: "project" }, [
    createBaseVNode("div", { class: "project-heading" }, [
      createBaseVNode(
        "h3",
        null,
        toDisplayString($props.project.name),
        1
        /* TEXT */
      ),
      createBaseVNode("button", {
        id: "vue-remove-" + $props.project.id,
        class: "remove",
        onClick: _cache[0] || (_cache[0] = withModifiers(($event) => $setup.emit("remove", $props.project.id), ["stop"]))
      }, "Remove", 8, ["id"])
    ]),
    createBaseVNode("p", { class: "muted" }, "In progress"),
    createBaseVNode("div", { class: "track" }, [
      createBaseVNode(
        "div",
        {
          class: "fill",
          style: normalizeStyle({ width: $props.project.progress + "%" })
        },
        null,
        4
        /* STYLE */
    ...
    createBaseVNode(
      "p",
      { class: "percentage" },
      toDisplayString($props.project.progress) + "%",
      1
      /* TEXT */
    )
  ]);
...
__component.__scopeId = "data-v-5d9e6b80";
var ProjectCard_default = __component;

// src/vue/App.vue
var __component2 = /* @__PURE__ */ defineComponent({
  __name: "App",
  setup(__props, { expose: __expose }) {
    __expose();
    const total = computed2(() => state.projects.length);
    const hostName = ref("Unreal Engine");
    const probeResult = ref("No C++ object");
    const host2 = findService("host");
    let timer = 0;
    onMounted(() => {
      timer = setInterval(() => state.ticks++, 1e3);
      if (host2) {
        hostName.value = host2.GetHostName();
        const probe = host2.GetProbeObject();
        probeResult.value = `${probe.GetIdentity()}:${probe.Add(19, 23)}`;
      }
    });
    onUnmounted(() => clearInterval(timer));
    const __returned__ = { total, hostName, probeResult, host: host2, get timer() {
      return timer;
    }, set timer(v) {
      timer = v;
    }, get state() {
      ...
      return reverseProjects;
    }, get removeProject() {
      return removeProject;
    }, get save() {
      return save;
    }, ProjectCard: ProjectCard_default };
    Object.defineProperty(__returned__, "__isScriptSetup", { enumerable: false, value: true });
    return __returned__;
  }
});
function render2(_ctx, _cache, $props, $setup, $data, $options) {
  return openBlock(), createElementBlock(
    ...
    {
      id: "vue-workspace",
      class: normalizeClass($setup.state.accent)
    },
    [
      createBaseVNode("div", { id: "vue-header" }, [
        createBaseVNode("div", null, [
          createBaseVNode("h1", null, "Vue / RmlUi"),
          createBaseVNode("p", { class: "muted" }, "Northstar workspace")
        ]),
        createBaseVNode(
          "span",
          {
          ...
          toDisplayString($setup.state.status),
          1
          /* TEXT */
        )
      ]),
      createBaseVNode("div", { id: "vue-sidebar" }, [
        createBaseVNode("h2", null, "Session settings"),
        createBaseVNode("label", { for: "vue-name" }, "Session name"),
        withDirectives(createBaseVNode(
          ...
          [vModelText, $setup.state.name]
        ]),
        createBaseVNode("label", { for: "vue-accent" }, "Accent"),
        withDirectives(createBaseVNode(
          "select",
          {
            id: "vue-accent",
            "onUpdate:modelValue": _cache[1] || (_cache[1] = ($event) => $setup.state.accent = $event)
          },
          [
            createBaseVNode("option", { value: "teal" }, "Evergreen"),
            createBaseVNode("option", { value: "blue" }, "Ocean"),
            createBaseVNode("option", { value: "rose" }, "Rose")
          ],
          512
          /* NEED_PATCH */
        ), [
          [vModelSelect, $setup.state.accent]
        ]),
        createBaseVNode("label", { for: "vue-intensity" }, "Intensity"),
        withDirectives(createBaseVNode(
          "input",
          {
            id: "vue-intensity",
            ...
            max: "100",
            step: "1",
            "onUpdate:modelValue": _cache[2] || (_cache[2] = ($event) => $setup.state.intensity = $event)
          },
          null,
          512
          /* NEED_PATCH */
          ...
          [
            vModelText,
            $setup.state.intensity,
            void 0,
            { number: true }
          ]
        ]),
        createBaseVNode("div", { class: "check" }, [
          withDirectives(createBaseVNode(
            "input",
            {
              id: "vue-enabled",
              type: "checkbox",
              "onUpdate:modelValue": _cache[3] || (_cache[3] = ($event) => $setup.state.enabled = $event)
            },
            null,
            512
            /* NEED_PATCH */
          ), [
            [vModelCheckbox, $setup.state.enabled]
          ]),
          createBaseVNode("label", { for: "vue-enabled" }, "Active session")
        ]),
        createBaseVNode("button", {
          id: "vue-save",
          class: "primary",
          disabled: $setup.state.busy,
          onClick: _cache[4] || (_cache[4] = (...args) => $setup.save && $setup.save(...args))
        }, "Save session", 8, ["disabled"]),
        createBaseVNode("div", { class: "sidebar-info" }, [
          createBaseVNode("p", { class: "muted" }, "COMPLETED SAVES"),
          createBaseVNode(
            "p",
            {
              id: "vue-saves",
            ...
            toDisplayString($setup.state.saves),
            1
            /* TEXT */
          )
        ])
      ]),
      createBaseVNode("div", { id: "vue-main" }, [
        createBaseVNode("div", { class: "section-title" }, [
          createBaseVNode(
            "h2",
            { id: "vue-preview-name" },
            toDisplayString($setup.state.name),
            1
            /* TEXT */
          ),
          createBaseVNode(
            ...
            toDisplayString($setup.state.enabled ? "Connected" : "Paused"),
            1
            /* TEXT */
          )
        ]),
        createBaseVNode("div", { id: "vue-metrics" }, [
          createBaseVNode("div", null, [
            createBaseVNode("span", { class: "muted" }, "Projects"),
            createBaseVNode(
              "p",
              {
                id: "vue-count",
              ...
              toDisplayString($setup.total),
              1
              /* TEXT */
            )
          ]),
          createBaseVNode("div", null, [
            createBaseVNode("span", { class: "muted" }, "Intensity"),
            createBaseVNode(
              "p",
              {
                id: "vue-percent",
              ...
              toDisplayString($setup.state.intensity) + "%",
              1
              /* TEXT */
            )
          ]),
          createBaseVNode("div", null, [
            createBaseVNode("span", { class: "muted" }, "Session time"),
            createBaseVNode(
              "p",
              {
                id: "vue-ticks",
              ...
              toDisplayString($setup.state.ticks) + "s",
              1
              /* TEXT */
            )
          ])
        ...
        createBaseVNode("div", { class: "section-title" }, [
          createBaseVNode("h2", null, "Activity"),
          createBaseVNode("div", { class: "commands" }, [
            createBaseVNode("button", {
              id: "vue-reverse",
              onClick: _cache[5] || (_cache[5] = (...args) => $setup.reverseProjects && $setup.reverseProjects(...args))
            }, "Reverse order"),
            createBaseVNode("button", {
              id: "vue-add",
              onClick: _cache[6] || (_cache[6] = (...args) => $setup.addProject && $setup.addProject(...args))
            }, "Add project")
          ])
        ]),
        createBaseVNode("div", { id: "vue-projects" }, [
          (openBlock(true), createElementBlock(
            Fragment,
            null,
            renderList($setup.state.projects, (project) => {
              return openBlock(), createBlock($setup["ProjectCard"], {
                key: project.id,
                id: "vue-project-" + project.id,
                project,
                onRemove: $setup.removeProject
              }, null, 8, ["id", "project", "onRemove"]);
            }),
            128
            /* KEYED_FRAGMENT */
          ))
        ]),
        createBaseVNode("div", { id: "vue-asset" }, [
          createBaseVNode("img", { src: "hello_world.png" }),
          createBaseVNode("div", null, [
            createBaseVNode("h3", null, "Interface reference"),
            createBaseVNode("p", { class: "muted" }, "RmlUi sample asset")
          ])
        ])
      ]),
      createBaseVNode("div", { id: "vue-footer" }, [
        createBaseVNode(
          "span",
          {
            id: "vue-host",
          ...
          toDisplayString($setup.hostName),
          1
          /* TEXT */
        ),
        createBaseVNode(
          ...
          toDisplayString($setup.probeResult),
          1
          /* TEXT */
        ),
        createBaseVNode(
...
__component2.render = render2;
var App_default = __component2;

// src/vue/main.ts
try {
  const instance = createApp(App_default);
  instance.mount();
