...
namespace
{
FString VueSampleManifest()
{
    const TSharedPtr<IPlugin> Plugin = IPluginManager::Get().FindPlugin(TEXT("RmlUiUnrealSamples"));
    return Plugin.IsValid() ? FPaths::Combine(Plugin->GetContentDir(), TEXT("Vue/current.json")) : FString();
}
...
bool FRmlVueMountTest::RunTest(const FString&)
{
    ...
    if (!TestTrue(TEXT("Real Puerts/V8 mounts compiled Vue SFC"), Runtime->Start(Widget.Get(), VueSampleManifest(), false))) return false;
...
bool FRmlVueIsolationTest::RunTest(const FString&)
{
    ...
    if (!A->Start(First.Get(), VueSampleManifest(), false) || !B->Start(Second.Get(), VueSampleManifest(), false)) return TestTrue(TEXT("Both VMs mount"), false);
...
public:
    ...
    bool Update() override
    {
        if (!Runtime.IsValid()) {
            ...
            if (!Test->TestTrue(TEXT("Watch fixture source mounts"), Runtime->Start(Widget.Get(), VueSampleManifest(), false))) return true;
