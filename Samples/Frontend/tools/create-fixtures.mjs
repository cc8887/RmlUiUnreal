...
const content = path.resolve(frontend, '../RmlUiUnrealSamples/Content/Vue');
...
for (const kind of ['update2', 'bad-hash', 'bad-script', 'bad-abi', 'bad-profile', 'bad-feature', 'bad-host-abi']) {
  const directory = path.join(root, kind);
  await mkdir(directory, { recursive: true });
  const manifest = structuredClone(original);
  manifest.version = kind === 'update2' ? 'remote-version-2' : kind;
  if (kind === 'bad-abi') manifest.abi = 999;
  if (kind.startsWith('bad-') && ['bad-profile', 'bad-feature', 'bad-host-abi'].includes(kind)) {
    if (!manifest.capabilities) throw new Error('Rebuild the Vue bundle to exercise capability rejection.');
    if (kind === 'bad-profile') manifest.capabilities.profile = 'slate-rhi';
    if (kind === 'bad-feature') manifest.capabilities.requiredFeatures.push('browser.dom');
    if (kind === 'bad-host-abi') manifest.capabilities.minimumHostAbi = 999;
  }
