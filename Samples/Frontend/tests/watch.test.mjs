...
test('SFC watch publishes changes and retains the last version on compile failure', { timeout: 40000 }, async () => {
  ...
  const core = path.resolve(frontend, '../../Frontend');
  const copy = path.join(fixture, 'Plugins/RmlUiUnreal/Frontend');
  const sampleCopy = path.join(fixture, 'Plugins/RmlUiUnreal/Samples/Frontend');
  await mkdir(copy, { recursive: true });
  await mkdir(sampleCopy, { recursive: true });
  for (const item of ['src', 'tools', 'package.json']) await cp(path.join(core, item), path.join(copy, item), { recursive: true });
  for (const item of ['src', 'assets', 'tools', 'package.json']) await cp(path.join(frontend, item), path.join(sampleCopy, item), { recursive: true });
  await symlink(path.join(core, 'node_modules'), path.join(copy, 'node_modules'), 'junction');
  await symlink(path.join(frontend, 'node_modules'), path.join(sampleCopy, 'node_modules'), 'junction');
  ...
  await cp(path.resolve(core, '../Tools/src'), path.join(compiler, 'src'), { recursive: true });
  await symlink(path.resolve(core, '../Tools/node_modules'), path.join(compiler, 'node_modules'), 'junction');
  const assets = path.join(fixture, 'Plugins/RmlUiUnreal/Content/RmlUi');
  await mkdir(assets, { recursive: true });
  await cp(path.resolve(core, '../Content/RmlUi/hello_world.png'), path.join(assets, 'hello_world.png'));
  const child = spawn(process.execPath, ['tools/build.mjs', '--watch'], { cwd: sampleCopy, windowsHide: true });
  ...
  try {
    ...
    const pointer = path.resolve(sampleCopy, '../RmlUiUnrealSamples/Content/Vue/current.json');
    const initial = await readFile(pointer, 'utf8');
    const app = path.join(sampleCopy, 'src/vue/App.vue');
